...
class OpponentEnv:
    ...
    def action_step(self, current_turn: int, owned_tiles: int):
            ...
            self._resources[0] += 1 + owned_tiles * 1
            self._resources[1] += 1 + owned_tiles * 1
            
            # 1. Economic Logic: Build soldiers if possible every 5 turns
            if current_turn % 1 == 0:
                can_afford_gold = self._resources[0] // 10
                can_afford_wood = self._resources[1] // 5
            
                num_to_build = int(min(can_afford_gold, can_afford_wood))

                if num_to_build > 0:
                    self._resources[0] -= 10 * num_to_build
                    self._resources[1] -= 5 * num_to_build
